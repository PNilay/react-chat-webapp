...
import CurrentPoll_Card from './CurrentPoll_Card';
...
function CurrentPolls() {
  ...
  const handleSubmit = async (card_index, result) =>{
    await updateDoc(doc(db, "polls_responce", polls[card_index].polls_responce_id), {
    ...
    <div>
      <div className="current__poll_list">
        {polls.map((poll, index) => (
          <CurrentPoll_Card  poll={poll} card_index={index} handleSubmit = {handleSubmit}/>
        ))}
      </div>
